...
const StartFragment: React.FC<ControllerProps> = ({ goNext }) => {
    ...
    return (
        <div className="relative h-screen">
            <video style={{ objectFit: "cover" }} className='absolute top-[-30%] w-screen h-screen' src="/aircraft.mp4" loop autoPlay muted controls={false} playsInline />
...
const TicketBottom: React.FC<ControllerProps> = ({ goNext }) => {
    ...
    return (
        <div className="bg-slate-100 rounded-xl p-5">
            ...
            <p onClick={goNext} className="p-3 max-w-[200px] mx-auto text-center text-white font-bold rounded-2xl bg-[#F1CB23] hover:bg-[#b99c1a] cursor-pointer">
                내게 맞는 동아리 찾아보기
            </p>
        </div>
    );
